...
from annotation.models import FeatureAnnotation, TYPE_MANUAL
...
class AnnotateDocumentView(DetailView):
    ...
    def get_queryset(self):
        sudbquery = FeatureAnnotation.objects.filter(
            final=True,
            type=TYPE_MANUAL,
            document=OuterRef('id')
        )
        documents = Document.objects.annotate(has_annotation=Exists(sudbquery))
        annotated_documents = self.request.session.get(
            'annotated_documents', [])
        return documents.filter(
            has_annotation=False).exclude(id__in=annotated_documents)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx.update({
            'progress': self.get_progress_for_user()
        })
        if self.object:
